...
from utils.app_paths import configure_playwright_env, log_root
...
class PlaywrightChapterCrawler:
    ...
    def _ensure(self) -> None:
        ...
        launch_result = ensure_managed_browser()
        self.logger.log(
            "chapter_fetch",
            "cdp_ready",
            {
    ...
    def close(self) -> None:
        try:
            if self._page is not None and (self._owns_page or os.getenv("WNS_CLOSE_PAGE") == "1"):
                self._page.close()
        except Exception:
            pass
        finally:
    ...
    def fetch_chapters(self, items: Iterable[dict], cleaner: Cleaner) -> Iterator[Chapter]:
        ...
        for item in planned:
            ...
            try:
                if nav_mode == "pager" and prev is not None and int(prev.get("volume_index") or 0) == vol_index and int(prev.get("chapter_index") or 0) + 1 == ch_index:
                ...
                self.logger.log("chapter_fetch", "chapter_ok", {"url": url, "volume_index": vol_index, "chapter_index": ch_index, "nav_mode": nav_mode})
                prev = item
                yield chapter
            except Exception as e:
                error_count += 1
                self.logger.log("chapter_fetch", "error", {"url": url, "error": str(e)})
                print(json.dumps({"type": "error", "chapter_url": url, "message": str(e)}, ensure_ascii=False), flush=True)
    ...
    def _try_nav_next(self, expected_url: str) -> bool:
        ...
        if locator is None:
            ...
            return False
        self.logger.log("chapter_fetch", "pager_next_attempt", {"from": old_url, "expected_url": expected_url})
        try:
