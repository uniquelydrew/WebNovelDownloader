...
class WorkspaceManager:
    ...
    def mark_completed(self) -> None:
        if not self._series_url:
            raise WorkspaceError("mark_completed requires a handle-bound WorkspaceManager(series_url=...)")
        paths = self.paths_for(self._series_url)
        ws = _read_json(paths.workspace_json, default={})
        ws["status"] = "completed"
        ws["last_error"] = None
    ...
    def mark_error(self, message: str) -> None:
        if not self._series_url:
            raise WorkspaceError("mark_error requires a handle-bound WorkspaceManager(series_url=...)")
        paths = self.paths_for(self._series_url)
        ws = _read_json(paths.workspace_json, default={})
        ws["status"] = "error"
        ws["last_error"] = str(message)
        ws["last_updated"] = _utc_iso()
        _atomic_write_json(paths.workspace_json, ws)
